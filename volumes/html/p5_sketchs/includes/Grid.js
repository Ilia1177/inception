...
class Grid {
	...
	init(size) {
		...
		if(p.windowWidth > max_width){
			this.ctx = p.createCanvas(max_width , p.windowHeight).parent("CNV0");
		} else if (p.windowWidth <= max_width) {
			this.ctx = p.createCanvas(p.windowWidth, p.windowHeight).parent("CNV0");
		}
		this.cellSize = size;
		...
		this.text_to_buffer("HAZARDOUS","center", 0, -45, 90);
		this.text_to_buffer("- ÉDiTORiAL -","center", 0, 45, 80);
		this.buffer_to_grid(180, 1);
		this.buffer.background(255);
		this.text_to_buffer("Analog","center", -160, 180, 80);
		this.buffer_to_grid(180, 2);
		this.buffer.background(255);
		this.text_to_buffer("|","center", 0, 180, 80);
		this.buffer_to_grid(180, 0);
		this.buffer.background(255);
		this.text_to_buffer("Digital","center", 160, 180, 80);
		this.buffer_to_grid(180, 3);
	}
